...
3. **解决矛盾**：
   ...
   - 4对1或3对2 → 采用多数数据
   - 全部不一致 → 用红色标识为不确定
4. **生成校验后的单一正确报告**（不是差异对比表）

**输出格式**：
- 每个模块生成一份 `[模块名称]_校验报告.md`
- 报告包含：确认正确的数据、不确定数据（红色标识）、数据质量评估
- **重要**：这是最终报告的直接数据源，不是差异对比表

**示例调用**：
```
【业务构成分析校验】请使用 diff-agent：
1. 读取5轮业务构成分析报告
2. 比对数据一致性
3. 解决矛盾（多数原则）
4. 生成《业务构成分析_校验报告.md》
...
对6个模块分别执行上述操作，生成6份校验报告。


#### 生成最终报告

满足上述条件后，**基于 diff-agent 生成的 6 份校验报告**（不是原始分析报告），按照 `references/report_template.md` 中的模板结构生成完整的投资研究报告：
...
- 详细解释差异原因，例如：轮次1和轮次2数据不一致，可能是因为轮次1数据是基于历史数据，而轮次2数据是基于实时数据。
